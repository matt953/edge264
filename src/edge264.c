...
Edge264Decoder *edge264_alloc(int n_threads, Edge264LogCb log_cb, void *log_arg, int log_mbs, Edge264AllocCb alloc_cb, Edge264FreeCb free_cb, void *alloc_arg)
{
	...
	// NAL types 24-31 are "unspecified" per H.264 spec - ignore them per issue #20
	for (int i = 24; i < 32; i++)
	...
	if (log_cb)
	{
		...
		dec->parse_nal_unit[0] = dec->parse_nal_unit[11] = dec->parse_nal_unit[12] = ignore_NAL_log;
		// NAL types 24-31 are "unspecified" per H.264 spec - ignore them per issue #20
		for (int i = 24; i < 32; i++)
...
int edge264_get_frame(Edge264Decoder *dec, Edge264Frame *out, int borrow)
{
	...
	{
		// For MVC, search queue 1 for frame with matching POC (Picture Order Count)
		// Find the CLOSEST match by FrameId to handle GOP boundaries correctly
		if (dec->ssps.BitDepth_Y != 0)
		{
			int target_poc = dec->FieldOrderCnt[0][pic0];
			int base_frame_id = dec->FrameIds[pic0];
			int best_diff = INT_MAX;
			for (int i = 0; i < 16; i++)
			{
				int pic = dec->get_frame_queue[1][i];
				if (pic >= 0 && dec->next_deblock_addr[pic] == INT_MAX &&
					dec->FieldOrderCnt[0][pic] == target_poc)
				{
					// Find dependent view with closest FrameId (same stereo pair)
					int frame_id_diff = dec->FrameIds[pic] - base_frame_id;
					if (frame_id_diff < 0)
						frame_id_diff = -frame_id_diff;
					if (frame_id_diff < best_diff)
					{
						best_diff = frame_id_diff;
						idx1 = i;
						pic1 = pic;
					}
				}
